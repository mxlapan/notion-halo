...
- **文章/页面专属 Emoji 图标**：可以在文章或页面设置中的「元数据」（Annotations）中添加自定义键值来配置专有图标。支持的键名为 `emoji` 或 `notion-halo/emoji`，值为任意 Emoji 字符（如 `🚀`）。配置后该图标将显示在标题上方，若文章有封面图，则会自动半重叠在封面图上，深度还原 Notion 体验。
